...
import { X, Save, AlertCircle, BookOpen, Award, Calendar, MessageSquare, Layers, Sparkles } from 'lucide-react';
import { studentService } from '../services/studentService';
import { computeStudentTotals, calculateGrade } from '../services/initialData';

const AdvancedReportCardDataModal = ({ isOpen, onClose, student, onSaveSuccess }) => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      console.error('Failed to save advanced report card modal data', err);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 font-body animate-fade-in">
      <div className="bg-white rounded-3xl max-w-4xl w-full max-h-[92vh] overflow-hidden shadow-2xl border border-gray-200 flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-heading font-extrabold text-lg text-white">Full Report Card Editor Model</h3>
                <span className="px-2 py-0.5 bg-gold/20 text-gold-light rounded text-[10px] font-bold border border-gold/30">
                  New Modal
                </span>
              </div>
              <p className="text-xs text-gray-200 mt-0.5">
                Student: <span className="font-bold text-white">{student.studentName}</span> (Class {student.className}-{student.section}, Roll #{student.rollNumber})
              </p>
            </div>
          </div>

          <div className="flex items-center gap-4">
            <div className="bg-maroon-dark/80 px-3 py-1.5 rounded-xl border border-maroon-light/30 text-right hidden sm:block">
              <div className="text-[9px] text-gold-light uppercase font-bold">Total Marks</div>
              <div className="text-sm font-extrabold text-white">
                {totalGrand.toFixed(2)} / {maxPossible} <span className="text-gold-light">({percentage}%)</span>
              </div>
            </div>
        ...
        <form onSubmit={handleSave} className="p-6 overflow-y-auto space-y-4 flex-1 text-xs">
            ...
            <div className="space-y-4">
              <div className="p-3 bg-amber-50 border border-amber-200 text-amber-900 rounded-xl flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span>Update Periodic Test, Notebook, SEA, Half Yearly, and Annual Exam scores.</span>
                </div>
                  ...
                  return (
                    <div key={idx} className="p-4 bg-gray-50 border border-gray-200 rounded-xl space-y-3">
                      <div className="flex items-center justify-between font-bold border-b border-gray-200 pb-2">
                        <span className="text-xs font-heading text-maroon uppercase">{sub.subject}</span>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                          <div>
                            <label className="block text-[10px] text-gray-600 font-bold mb-0.5">Per Test (10)</label>
                            <input
                          ...
                          <div>
                            <label className="block text-[10px] text-gray-600 font-bold mb-0.5">NB (5)</label>
                            <input
                          ...
                          <div>
                            <label className="block text-[10px] text-gray-600 font-bold mb-0.5">SEA (5)</label>
                            <input
                          ...
                          <div>
                            <label className="block text-[10px] text-gray-600 font-bold mb-0.5">Half Yearly (80)</label>
                            <input
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                          ...
                          <div>
                            <label className="block text-[10px] text-gray-600 font-bold mb-0.5">Annual Exam (80)</label>
                            <input
          ...
          {activeTab === 'coScholastic' && (
            <div className="space-y-4">
              <h4 className="font-heading font-bold text-xs text-maroon uppercase border-b border-gray-200 pb-1">
                Co-Scholastic Activity Evaluation Grades
              </h4>
              <div className="space-y-3">
                {coScholastic.map((item, idx) => (
                  <div key={idx} className="p-3 bg-gray-50 border border-gray-200 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-3 flex-1 w-full">
                      <div>
                        <label className="block text-[10px] text-gray-500 font-bold mb-0.5">Half Yearly Grade</label>
                        <select
                          value={item.hyGrade || 'A'}
                          onChange={(e) => handleCoScholasticChange(idx, 'hyGrade', e.target.value)}
                          className="w-full p-2 bg-white border border-gray-300 rounded-lg focus:border-maroon font-bold text-maroon text-xs"
                        >
                      ...
                      <div>
                        <label className="block text-[10px] text-gray-500 font-bold mb-0.5">Annual Grade</label>
                        <select
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl space-y-2">
                  <div className="font-bold text-maroon text-xs border-b border-gray-200 pb-1">Term 1 Attendance</div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[10px] text-gray-600 font-bold mb-0.5">Present Days</label>
                      <input
                    ...
                    <div>
                      <label className="block text-[10px] text-gray-600 font-bold mb-0.5">Total Days</label>
                      <input
                ...
                <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl space-y-2">
                  <div className="font-bold text-maroon text-xs border-b border-gray-200 pb-1">Half Yearly Attendance</div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[10px] text-gray-600 font-bold mb-0.5">Present Days</label>
                      <input
                ...
                <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl space-y-2">
                  <div className="font-bold text-maroon text-xs border-b border-gray-200 pb-1">Term 2 Attendance</div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[10px] text-gray-600 font-bold mb-0.5">Present Days</label>
                      <input
                ...
                <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl space-y-2">
                  <div className="font-bold text-maroon text-xs border-b border-gray-200 pb-1">Annual Attendance</div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[10px] text-gray-600 font-bold mb-0.5">Present Days</label>
                      <input
          ...
          {activeTab === 'summary' && (
            <div className="space-y-4">
              <h4 className="font-heading font-bold text-xs text-maroon uppercase border-b border-gray-200 pb-1">
                Remarks & Result Promotion Status
              </h4>

              <div className="space-y-3">
                <div>
                  <label className="block font-bold text-gray-700 mb-1 text-xs">Class Teacher Remarks</label>
                  <textarea
                    rows={3}
                    value={remarks}
                ...
                <div>
                  <label className="block font-bold text-gray-700 mb-1 text-xs">Result Status</label>
                  <select
                    value={resultStatus}
                    onChange={(e) => setResultStatus(e.target.value)}
                    className="w-full p-3 bg-white border border-gray-300 rounded-xl focus:border-maroon focus:ring-1 focus:ring-maroon font-bold text-maroon text-xs"
                  >
          ...
          <div className="pt-4 border-t border-gray-200 flex items-center justify-between">
            <div className="text-xs text-gray-500 font-semibold">
              Live Total: <span className="font-extrabold text-maroon">{totalGrand.toFixed(2)}</span> ({percentage}%)
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                type="submit"
                disabled={saving}
                className="px-6 py-2 bg-maroon text-white font-extrabold rounded-xl text-xs hover:bg-maroon-dark flex items-center gap-2 shadow-md disabled:opacity-50"
              >
                <Save className="w-4 h-4" />
                <span>{saving ? 'Saving...' : 'Save & Refresh Report'}</span>
              </button>
            </div>
          </div>
...
export default AdvancedReportCardDataModal;
